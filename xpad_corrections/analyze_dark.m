...
for cap_idx=1:num_caps
  for asic_idx=1:asic_count
    ...
    asic_row_lower = asic_row_idx*asic_height+1+y_margin;
    asic_row_upper = (asic_row_idx+1)*asic_height - y_margin;
    asic_col_lower = asic_col_idx*asic_width+1+x_margin;
    asic_col_upper = (asic_col_idx+1)*asic_width - x_margin;

    ## Get the stack for the ASIC and cap
    asic_stack = dark_raw(asic_row_lower:asic_row_upper, asic_col_lower:asic_col_upper,cap_idx:num_caps:num_dark_frames);
    ...
    figure(1)
    plot(pair_array, pair_std, "r-*;Measured Noise;", pair_array, y_reg, 'g-s;Projected Noise;', fixed_noise_x, fixed_noise_y, "v-v;Calculated Fixed Noise;")
