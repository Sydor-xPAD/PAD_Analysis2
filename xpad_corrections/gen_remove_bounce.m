...
bg_light = raw_light;
bg_light = bg_light(129:256,257:512,:);

bg_mean = mean(reshape(bg_light(70:120,70:120,:), 51*51, []), 1);

for frame_idx = 1:(num_frames/num_caps)
  db_light(:,:,frame_idx) = bg_light(:,:,frame_idx) - bg_mean(frame_idx);
endfor

chunk = db_light(10:50,20:120,:);
strip = reshape(chunk, 41*101, []);
strip_raw = reshape(bg_light(10:50,20:120,:), 41*101, []);
std(strip)
std(strip_raw)
