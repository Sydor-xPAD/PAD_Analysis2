...
class dataObject:
   ...
   def  openRunAndCreateData( self,cap ):
      ...
      backFile = kega-blank_00000001.raw;
      foreFile = fg_7_00000001.raw;
   ...
   def  openRunAndCreateData_DC( self ):
      ...
      backFile = 'run-dark_5ms/frames/dark_5ms_00000001.raw';
      foreFile = 'run-Bright_5ms/frames/Bright_5ms_00000001.raw';
      imgType='KECKPADX2'

      if imgType=='KECKPADX2':
         imgSize = (512,1024)
      else:
         imgSize = (512,512)    # Ignoring case of a geocorrected image
      ...
      cwd = os.getcwd()
      back = BKL.KeckFrame(backFile, imgType='KECKPADX2')
      fore = BKL.KeckFrame(foreFile, imgType='KECKPADX2')

      numImagesF = fore.numImages
      numImagesB = back.numImages


      foreStack = np.zeros((8,512,1024),dtype=np.double)
      backStack = np.zeros((8,512,1024),dtype=np.double)
      FFStat = "noFF"
      if self.FFImage ==1 :
         fileObject = open(cwd + "/pickleFF.pi", 'rb')  # not at all tested will break 4 sure
         ffCorect = pkl.load(fileObject)
      ...
      for fIdex in range(numImagesB):
         (mdB,dataB) = back.getFrame()
         backStack[mdB.capNum-1,:,:] += np.resize(dataB,imgSize)
      ...
      for fIdex in range(numImagesF):
         ...
         foreStack[mdF.capNum-1,:,:] += np.resize(dataF,imgSize)
